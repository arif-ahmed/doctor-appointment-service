...
public class CreateAppoinmentCommandTests
{
    ...
    public async Task CreateAppoinmentCommand_Should_Return_String()
    {
        ...
        appointmentRepositoryMock.Setup(x => x.AddAsync(It.IsAny<Appointment>())).ReturnsAsync(new Appointment { Id = Guid.NewGuid().ToString(), PatientName = command.PatientName, DoctorId = command.DoctorId, AppointmentDate = command.AppointmentDate });
